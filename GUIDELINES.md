...
- **Context**:
  ...
  - _Note_: The middleware filters requests without valid Tenant IDs, so handlers/services can assume it exists. Do NOT manually check for empty ID.
  - **Updates**: When implementing `Update` handlers, explicitly exclude immutable fields (like `Type`, `Currency` for Accounts) in the Repository layer to ensure data integrity.
...
- **Repository Layer**: ALL methods (Get, List, Update, Delete) MUST accept `tenantID` as an argument and filter by `tenant_id = $x` in the SQL query.
  - _Exception_: Global lookups (if any) or user-scoped queries that are not tenant-bound.
...
- `@Accept`, `@Produce`: Usually `json`.
  - _Exception_: The `Login` endpoint uses `multipart/form-data` (annotated as `mpfd`).
...
_(Currently in early stages, but aim for the following)_
